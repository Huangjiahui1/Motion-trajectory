...
#按照不同大小的缩放尺寸视频进行分类
path = 'F:/sheepvedios/玛曲跑步/主视频/模型/csv'
#path = ''
namelist_k = file_name_k(path)
k_l = {'148':[],'131.03':[],'127':[],'147':[],'128':[]}
...
if __name__ == '__main__':
    pose_path ='F:/sheepvedios/玛曲跑步/主视频/模型/csv/all'
...
#前处理完成，开始后续分析
    ...
    for i in range(len(idic)):
        try:
            ...
            point = p_class.lhoof
            #提取name
            name = pose_path_fir[int(id1)].split('all/')[-1].split('DLC')[0]
    ...
    #excel的格式化输出
    excel = pd.ExcelWriter('MQ_T_all_V_0610.xlsx')
    ...
    #步态周期过滤
    df_alldata_str_cycle01_V1to10 = df_alldata_str.query('"0.5">cycle_time>"0.1" or V=="-"').query('("9">V>"1") or (V =="-")')
    ...
    V_max_df.to_excel('MQ_T_name_V_0610.xlsx')
