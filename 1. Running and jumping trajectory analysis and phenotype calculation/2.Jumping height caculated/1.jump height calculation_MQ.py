...
from filt_pretreatpoint_tendst_MQ import * 
...
pre = r'F:\sheepvedios\output\jump_2.2C\MQ_jump\MQ_jump_12'
pre = r'F:\softln\DLC1\output\jump_2.2C\MQ_jump\MQ_jump_7.5'
pre = r'F:\softln\DLC1\output\jump_2.2C\MQ_jump\MQ_jump_6'
...
for i in range(len(right_spin_list)):
    name_r = right_spin_list[i].split('/')[-1].split('\\')[-1].split('DLC')[0]
    rlist.append(name_r)
for i in range(len(right_spin_list_1)):
    name_r = right_spin_list_1[i].split('/')[-1].split('\\')[-1].split('DLC')[0]
    rlist.append(name_r)    

for i in range(len(left_spin_list)):
    name_l = left_spin_list[i].split('/')[-1].split('\\')[-1].split('DLC')[0]
    llist.append(name_l)
...
def benifit_v5(new,s,t,individualname=''):
    ...
    #求实根验证 MQ，XH互换
    max_x, max_y, valid_critical_points, valid_inflection_points, is_max_x_a_root = evaluate_model_in_range(best_model, x[-1], x[0])
...
def model(path, wind=12, order=6,n_neighbors=10,individual = 2,OD = 'none', flag_nonereal=0, plot=False, individualname='',llist=llist,rlist=rlist):
    ...
    if path.split('\\')[-1].split('DLC')[0] in rlist:
        print('视频顺时针旋转1.5')
        new = spin.spin_r(new, 1.5)
    elif path.split('\\')[-1].split('DLC')[0] in llist:
        print('视频逆时针旋转3.1')
...
#    print('最小值为'+str(miny))
    return miny,dy,maxpoint,avgy,distance,minp_average,normal_x,normal_y,best_max_x,miny,best_model, best_degree
'''        
...
def model_plot(x,y,best_max_x,best_max_y,best_model,best_degree,individualname=''):
    # 可视化最佳拟合模型
    plt.figure(figsize=(12, 8))
    plt.scatter(x, y, color='gray', label='Original Data', alpha=0.5)
    x_smooth = np.linspace(x.min(), x.max(), 500)
...
if __name__ == '__main__':
    ...
    for d in range(len(L)):     #**************************************************************************************************
        if L[d][-3:] != 'csv':
            continue    
        miny1,dy1,maxpoint1,avgy1,distance1,minp_average1,x1,y1,best_max_x1,best_max_y1,best_model1,mp1,mpdy1 = [0]*13
        miny2,dy2,maxpoint2,avgy2,distance2,minp_average2,x2,y2,best_max_x2,best_max_y2,best_model2,mp2,mpdy2 = [0]*13
        mpdy1,mpdy2 = [0,0]
        avg1, avg2 = [0,0]
    ...
    ——————————————————————————————————————————————————————————————————————————
        ...
        miny2 = 0
        try:
            miny1,dy1,maxpoint1,avgy1,distance1,minp_average1,x1,y1,best_max_x1,best_max_y1,best_model1,best_degree1 = model(L[d],wind=wind,n_neighbors=10,OD = 'none',individual=1,individualname=name1)
        ...
        try:
            ...
            miny2,dy2,maxpoint2,avgy2,distance2,minp_average2,x2,y2,best_max_x2,best_max_y2,best_model2,best_degree2 = model(L[d],wind=wind,n_neighbors=10,OD = 'none',individual=2,individualname=name2)
        except (ValueError,IndexError):
            flag_nonereal_2=1
        ...
        if miny1 == 0 and miny2 != 0:
            if name2 == 0:    
                name2 = name1
        try:
            model_plot(x1, y1, best_max_x1, best_max_y1, best_model1, best_degree1, name1)
        except (AttributeError,NameError):
            pass
        try:
            model_plot(x2, y2, best_max_x2, best_max_y2, best_model2, best_degree2, name2)
    ...
    #result_df.to_excel('F:\项目数据\运动GWAS项目绘图\附图\DLCMQ_jump_7.5_20240607.xlsx')
    #result_df.to_excel('F:\项目数据\运动GWAS项目绘图\附图\DLCMQ_jump_6_20240611.xlsx')
            #提取打包
