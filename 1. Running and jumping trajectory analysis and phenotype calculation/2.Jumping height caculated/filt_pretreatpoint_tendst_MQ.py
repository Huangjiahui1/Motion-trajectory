...
def pretreatpoint(path,individual=1):
    ...
    try:
        ...
        new1 = filt(i1p2)
        new2 = filt(i2p2) 
        if 'M54_2813_47_0096_3' in path or 'M19_0689_46_8498_3' in path or 'M57_0054_43_2849_3' in path or 'M60_0633_6_0406_3' in path or 'T60_0633_6_0406_3' in path:
            new1 = LOF(new1,n_neighbors=5)  #MQ6M开启
            new2 = LOF(new2,n_neighbors=5)  #
    except ValueError:        
        if individual==2:
            raise ValueError
...
#            s = down[i-1] 
    ...
    try:
        s_x =newf.query('x >= 1400')['f'].iloc[-1] #t_x =newf.query('x >= 1250')['f'].iloc[0]   #6M可以用1400-750  
    except IndexError:
        print('x=1400以后该序列没有值')
        s_x = newf['f'].iloc[-1] 
    try:
        t_x = newf.query('x <= 750')['f'].iloc[0]
    except IndexError:
        t_x = newf.query('x <= 800')['f'].iloc[0]
    try:
...
def high(new,s=0,t=0):
    ...
    for i in range(len(new)):   #纵向：10M 355-570  根据MQ_6M调整：
         ...
         #   avglist.append(new[i][2])
        #MQ6M
        if (t <= new[i][0] <= t+30) and ((250 <= new[i][2] <= 800)):
            #print(s,s-50,new[i][0])
            avglist.append(new[i][2])     
        #if (s-30 <= new[i][0] <= s) and ((250 <= new[i][2] <= 800)):
        #    #print(s,s-50,new[i][0])
        #    avglist.append(new[i][2])
