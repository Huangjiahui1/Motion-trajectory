...
def filt(point):
    ...
    new = []
    for i in point:  
        if (i[3] == '') or (i[1] == '') or (i[2] == '') or (float(i[2]) < 300) or (float(i[2]) > 750):
            continue
        else:
            new.append(i)
    new1 = []
...
def pretreatpoint(path,individual=1):
    ...
    '''
    #从名字中计算该次视频有几个个体
    namecount = len(path.split("/")[-1].split('DLC')[0].split('_'))
    if namecount == 3:
        indi = 1
    elif namecount == 5:
        indi = 2
    ...
    try:
        ...
        if datalist[0][1] > datalist[0][7]:
            for i in range(3,len(datalist)):
                i1p1.append([datalist[i][0],datalist[i][1],datalist[i][2],datalist[i][3]])
                i1p2.append([datalist[i][0],datalist[i][4],datalist[i][5],datalist[i][6]])
        if datalist[0][1] < datalist[0][7]:
            for i in range(3,len(datalist)):
                i1p1.append([datalist[i][0],datalist[i][7],datalist[i][8],datalist[i][9]])
                i1p2.append([datalist[i][0],datalist[i][10],datalist[i][11],datalist[i][12]])
        i1p1 = i1p1[3:];i1p2 = i1p2[3:];i2p1=0;i2p2=0
        new1 = filt(i1p2)
        new2 = np.array([])
        
        C = Change(new1, new2, indi=indi)
        C.combin()
        new1 = C.new_seq1[:,:-1]
...
def tendst(new,wind):
    ...
    #低通滤波器
    b, a = signal.butter(1, 0.7, 'lowpass')   #配置滤波器
...
#            s = down[i-1] 
    ...
    try:
        s_x =newf.query('x >= 750')['f'].iloc[0] #t_x =newf.query('x >= 1250')['f'].iloc[0]   #6M可以用1400-750  
    except IndexError:
        print('x=1250以后该序列没有值')
        s_x = newf['f'].iloc[0] 
    t_x = newf.query('x <= 1250')['f'].iloc[-1]
    try:
...
def flight(new,s,t):
    ...
    new.columns = ['f','x','y','p']
    s_x = new.query('f >= @s')['x'].iloc[0]
    t_x = new.query('f >= @t')['x'].iloc[0]
    d = t_x - s_x
    return s_x,t_x,d
...
if __name__ == '__main__':
    ...
    path = '/home/hjh/DLC1/output/jump_2.2C/XH_jump/XH_jump_7.5/T8_020_4_068_3DLC_dlcrnetms5_XH_jump_7.5_2.2CNov8shuffle1_60000_el_filtered.csv'
